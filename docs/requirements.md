...
| OTF-3 | `usb attach-host` / `usb attach-target`: software flip of the A-port mux (MS2109 GPIO) — hands-free physical media (image stick host-side, boot it target-side, BIOS-visible) | S | ☐ | **still blocked for *writing* the mux, but no longer opaque**: the switch position is now readable with no patch at XDATA `0xDF00` bit 0 (0=host, 1=target), confirmed across four flips. The read-only ROM `0xc5` SFR probe has since been run and **failed** — the MS2109 does not implement it (echo, then `00` at every SFR including SP and ACC), so the no-patch SFR route is closed. Remaining leads: disassemble our own dumped EEPROM image, then the USB capture of the vendor app. See findings 7-9 |

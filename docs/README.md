...
| [Provisioning a Linux control host](https://github.com/curtisgalloway/paniolo/blob/main/docs/control-host-provisioning.md) | **Design, not implemented**: the unattended-install seed for a new control host (x86 mini-PC or Raspberry Pi 4/5) — one shared cloud-init core, two wrappers (Ubuntu autoinstall USB / Pi NoCloud boot-partition files), fetching the release `.deb`; the seed's job ends at first SSH, then the lab-file flow takes over. Host sizing guidance included. |

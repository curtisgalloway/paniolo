...
| [Openterface deep control — findings & testing TODO](https://github.com/curtisgalloway/paniolo/blob/main/docs/openterface-deep-control.md) | **TODO, testing gates everything (tracker §6.1 OTF-1)**: the Mini-KVM's open schematic shows a software-switchable USB-A port (MS2109 GPIO mux), DTR-driven soft unplug/replug of the A-port device, RTS hardware reset of the CH9329, a writable descriptor EEPROM, and developer extension pins exposing a target-side gadget slot. Bench-verification checklist + planned `usb attach-*` / `usb replug` verbs. |

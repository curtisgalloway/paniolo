...
| CORE-14 | Out-of-tree **hardware plugins** for private/unreleased bench hardware: named per target (`[[plugin]]`), run on their host with args appended, self-describing to agents (`plugin describe`), nothing about the hardware in this repo | S | ☑ | `paniolo plugin add/set/rm/list/describe/run`; `PluginChannel` in `cli/src/model.rs`, routed by name like `serial`; [plugins.md](../plugins.md) |

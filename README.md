...
| [Hardware plugins](https://curtisgalloway.github.io/paniolo/plugins/) | `paniolo plugin` | Drive private or unreleased bench hardware (a custom panel that presses a board's buttons, a strap controller, a JTAG mux) through an out-of-tree command you own — named per target, run on its host with your arguments appended, self-describing to agents via `plugin describe`; nothing about the hardware needs to be public |

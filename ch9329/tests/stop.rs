...
//! `ch9329 stop` shuts the daemon down through its authenticated `POST /stop`
...
fn authenticated_stop_exits_and_removes_discovery() {
    let dir = std::env::temp_dir().join(format!("ch9329-stop-integration-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    // No hardware: the UART is opened lazily on the first command, so a
    // control device nothing matches keeps the daemon fully functional over
    // HTTP without ever touching a port.
    let child = Command::new(env!("CARGO_BIN_EXE_ch9329"))
        .args([
            "--device",
            "/dev/no-such-ch9329-device-zz",
            "serve",
            "--port",
            "0",
        ])
    ...
    let result = Command::new(env!("CARGO_BIN_EXE_ch9329"))
        .arg("stop")
